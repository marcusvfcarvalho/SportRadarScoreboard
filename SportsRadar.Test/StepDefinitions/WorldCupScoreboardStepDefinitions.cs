...
public class WorldCupLiveScoreStepDefinitions
{
    ...
    public void ThenIGetTheFollowingScore(Table table)
    {
        ...
        Assert.Multiple(() =>
        {
            Assert.AreEqual(table.Rows.Count, listMatches!.Count);
            Assert.AreEqual(Convert.ToInt32(table.Rows.First()[2]), listMatches!.First().HomeTeamScore);
            Assert.AreEqual(Convert.ToInt32(table.Rows.First()[3]), listMatches!.First().AwayTeamScore);
